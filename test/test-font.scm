...
;; hpdf-get-font
;; 
(test-section "hpdf-get-font")

(define (mktest desc expect name enc)
  (let* ([pdf (hpdf-new)])
    (test* desc expect  (class-of (hpdf-get-font pdf name enc)))))
...
(map (^ (entry)
       (let* ((name (~ entry 0))
	      (msg (format #f "default font ~a" name)))
	 (mktest msg <hpdf-font> (~ entry 0) ""))) def-font)
